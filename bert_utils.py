...
bert_tokenizer = transformers.BertTokenizer.from_pretrained(
    "huggingface_models/bert-base-uncased", local_files_only=True
)
bert_model = transformers.BertModel.from_pretrained(
    "huggingface_models/bert-base-uncased",
...
def _encode_text(
    ...
    """
    batch_encoding_tokenized: transformers.BatchEncoding = tokenizer(
        text, return_tensors="pt", add_special_tokens=special_tokens
    )
    bert_input_ids: torch.Tensor = batch_encoding_tokenized[
        "input_ids"
    ].squeeze()
    tokenized_text = tokenizer.convert_ids_to_tokens(bert_input_ids.tolist())
...
def get_attention_to_target_word_in_sentence(
    ...
    tokenized_text, text_bert_ids = _encode_text(
        sentence, bert_tokenizer, special_tokens=True
    )
...
def extract_vector_hidden_state(
    ...
    _, target_word_token_ids = _encode_text(
        target_word, bert_tokenizer, special_tokens=False
    )

    # Extract token_ids of sentence
    _, sentence_token_ids = _encode_text(
        sentence, bert_tokenizer, special_tokens=True
    )
