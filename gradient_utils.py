...
def get_salience_scores(data, model):
    ...
    for input_ids, token_type_ids, attention_mask, label in zip(data["input_ids"], 
                                                                data["token_type_ids"], 
                                                                data["attention_mask"],
                                                                data["label"]):
        ...
        preds = model(input_ids = torch.tensor([input_ids]),
        ...
        input_ids = [input_id for input_id,  mask in zip(input_ids, attention_mask) if mask ==1]
        grad = embeddings.word_embeddings.weight.grad[input_ids].detach().numpy()
        ...
        # grad_norm
        grad_norm_ = np.linalg.norm(grad, axis=1)# Frobenius Norm#math.sqrt(sum(grad[0]**2))
        grad_norm_ /= np.sum(grad_norm_) # normalize
        grad_norm.append(grad_norm_)
        
        # grad_dot_input
        grad_dot_input_ = np.sum(grad* weights, axis = -1)
        grad_dot_input_ = grad_dot_input_ + np.finfo(np.float32).eps # necessary? https://github.com/PAIR-code/lit/blob/main/lit_nlp/components/citrus/utils.py
        grad_dot_input_ = grad_dot_input_ / np.abs(grad_dot_input_).sum(-1) # normalize
...
def plot_grad_norm(data, save_path):
    ...
    for i, (tokens,grad_norm) in enumerate(zip(data["tokens"], data["grad_norm"])):
...
def plot_grad_dot_input(data, save_path):
    ...
    for i, (tokens,grad_dot_input) in enumerate(zip(data["tokens"], data["grad_dot_input"])):
